...
export function ProductPanel() {
  ...
  const [message, setMessage] = useState("Demo state is ready.");
  ...
  const loadProducts = async () => {
    try {
      ...
      setProducts(data);
      setMessage(`Loaded ${data.length} product(s) from API.`);
    } catch (error) {
  ...
  const createProduct = async () => {
    try {
      ...
      setMessage(`Created ${created.name} (#${created.id})`);
      setName("");
      setPrice("");
    } catch (error) {
      const detail = error instanceof Error ? error.message : "Unexpected error";
  ...
  return (
    <section className={styles.panel}>
      <h2>Products API Playground</h2>
      <p>This docs panel demonstrates shared `InputField` usage and API wiring.</p>
